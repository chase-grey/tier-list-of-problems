...
Write-Output "Reading JSON file..."
# Read the existing JSON file
$jsonContent = Get-Content -Path "c:\EpicSource\tier-list-of-problems\src\assets\pitches.json" -Raw

Write-Output "Removing closing bracket..."
# Remove the closing bracket from the JSON content
$jsonContent = $jsonContent.TrimEnd().TrimEnd("]")

Write-Output "Adding new pitch entries..."
# Append the new pitches
$jsonContent += ",`n$pitch1,`n$pitch2,`n$pitch3,`n$pitch4`n]"

Write-Output "Writing updated JSON file..."
# Write the updated JSON back to the file
$jsonContent | Set-Content -Path "c:\EpicSource\tier-list-of-problems\src\assets\pitches.json" -Encoding UTF8

Write-Output "Successfully added 4 missing pitches to the JSON file."
